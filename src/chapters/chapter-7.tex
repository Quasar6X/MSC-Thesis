...
komponenseket, a \textit{Gekko} komponenes pedig egy opcionális nulladik lépés, ennek ellenére az utolsó komponens
fejezetben ismertetem.

Ennek legfőbb okai, hogy a \textit{Gekko} komponenes használata opcionális, és a~\ref{chp:reporter} fejezetben
megismert szoftverkomponenshez hasonlóan egy külön parancsosori futtatható.

A \textit{Gekko} komponenes célja, hogy egy megfelelő teszt generátor alkalmazást felhasználva, létrehozni egy olyan
...
generikusabb és robosztusabb megoldást választottam.
...
A \textit{Gekko} komponens szoftver architektúrális szempontból fel van készítve arra, hogy egyéb teszt generátorokat
...
\section{A komponenes konfigurácjója}

    A \textit{Gekko} szoftver komponenes újrahasználja a~\ref{chp:config-manager} fejeyetben megismert
    ...
    \begin{itemize}
        \item \textit{DEFAULT}: itt adahatóak meg \textit{Gekko} konfigurációi. A \verb|COMPILE_TIMEOUT| és a
        ...
        \item \textit{CSMITH}: A jelenleg támogatott generátor kapcsolói. Itt adható meg az \verb|OPTIONS| kapcsolóval
              ...
              időkeretbe.
        \item A további szekciókban a verfikáló fordítóprogramokat lehet felsorolni a \textit{DEFAULT} szekció
    ...
    A \textbf{\ref{fig:gekko-config}. ábrán} látható egy példa a konfigurációs fájlra.
...
\section{A komponens felépítése}
    ...
    kapcsoló szemantikailag nem összekeverndő, az összesen generálni kívánt tesztesetek számát szabályzó
    ...
    A \textit{Gekko} komponenes viszont egyszerre dupla annyi feladatot tesz fel az esemény hurokra mint amennyi
    egyidejűleg futhat.
    ...
    azért van, hogy a feladatok számából következő lineárisan növekvő tráigény, felülről korlátos legyen. Továbbá, ha
    gyorsabban regisztrálódnak feladatok az esemény hurkon mint ahogyan elfogynak, akkor a \textit{Szemaforért} is
    zárolási verseny alakulna ki, ezt az esetet is kizárja a felsőkorlát beiktatása.
    ...
    \begin{enumerate}
        ...
        \item Teszteset verfikáció, a konfigurációban definiált fordítóprogramokkal.
        \item Verfikált teszteset transzformáció, a \textit{SuperSet} teszt keretrendszer formátumára.
    \end{enumerate}
    ...
    A komponens megvizsgálja a verifikáló fordítóprogramokat pár egyszerű invokációval, így megbizonyosdik róla hogy a
    parancssori argumentum interfészük a \textit{Gekko} által ismert. A komponens ezen felül több a futás során
    szükséges elérési útvonal létrehozásáról is gondoskodik, például a kimeneti mappájéról, és a \textit{log} fájlok
    mappájéról.
...
\section{A \textit{Gekko} alkomponensei}
    ...
    generál egy tesztesetet. Ezt a konfigurációban megadott, teszteset generátor alkamazás alprocesszusban történő
    meghívásával teszi.

    A második komponenes az \textit{EvaluationEngine}, vagyis a kiértékelő komponens. Ez a komponens végzi a
    \textit{TestGenerator} által generált tesztesetek verfikációját. A konfigurációban megadott fordítóprogramokkal
    lefordítja a tesztesetet.

    Azonban itt nem fejezi be a munkát. A tény, hogy minden alkalmazott fordítoprogram tudott futtatható állományt
    generálni, még nem elég ahhoz, hogy meggyőzödjünk a teszteset helyességéről.

    A \textit{Csmith}~\cite{csmith} teszteset generáló program kimeneti forrásfájlai futásidejű tesztesetek.
    Kimenetükben egy ellenőrzőössszeget adnak vissza. Ennek az összegnek a helyességét kell verifikálni. Ennek
    ...
    információk, mert nem kell a tesztesetet lefordított futtható állományát elmenteni, ha reprodukálni szeretnénk a
        ...
        \begin{minted}{json}
{
    ...
    \subsection{Az alkomponens interfészek}
        ...
        A protokollok egy szoftverarchitektúrális szempontból, is előnyesek, mert egy tisztább megoldást kínálnak mint
