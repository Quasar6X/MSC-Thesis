\section{\textit{SuperSet}, \textit{SuperSetProducer}, és \textit{SuperSetConsumer}}
    ...
    A \textit{SuperSet} komponens, ahogy azt a~\ref{sec:intro-superset} fejezetben említettem, a lokális futtatást
    oldaja meg. A \textit{SuperSetConsumer} (röviden \textit{Consumer}) pedig, ahogyan
    a~\ref{sec:intro-superset-prod-cons} fejezet is említi, az elosztott működésben a kliens oldalt valósítja meg.
    ...
    \subsection{A munkafolyamatok}
        ...
        esetében pedig egy munkafolyamat jön létre. A munkafolyamatokat az \textbf{\ref{fig:superset-workflows}. ábra
        mutatja}.
...
async def begin_client_work(
    ...
    try:
        ...
        \subsubsection{Összegzés}
            ...
            tudatosan nem a \textit{HTTP}~\cite{http} protokollt választottam mert az sok fölösleges plusz információt
            is átküld. Az implementált protokoll tökéletesen modellezi az elosztott keretrendszer invarianciáit, és
            minimális adatot küld.
